...
class Main12Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var movie_id = intent.getStringExtra("ids")

        similarrView.apply {
            layoutManager = LinearLayoutManager(this@Main12Activity, RecyclerView.VERTICAL,false)
            adapter = similaradapter
        }
        similaradapter.onItemClick = {
            val i = Intent(this,Main10Activity::class.java)
            i.putExtra("movieid",it.id.toString())
            startActivity(i)
            ...
            if (response.isSuccessful){
                if (response.body()?.results.isNullOrEmpty()){
                    GlobalScope.launch(Dispatchers.Main) {
                        Toast.makeText(this@Main12Activity,"Trailer not available",Toast.LENGTH_LONG).show()
                    }
                }
