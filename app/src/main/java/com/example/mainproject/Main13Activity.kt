...
class Main13Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tv_id = intent.getStringExtra("ids")

        similarrView.apply {
            layoutManager = LinearLayoutManager(this@Main13Activity, RecyclerView.VERTICAL,false)
            ...
            if (response.isSuccessful){
                if (response.body()?.results.isNullOrEmpty()){
                    GlobalScope.launch(Dispatchers.Main) {
                        Toast.makeText(this@Main13Activity,"Trailer not available",Toast.LENGTH_LONG).show()
                    }
                }
